$(document).ready(function () {
  ...
  function updateQuestionNumbers() {
      $("#questionsContainer .question").each(function (index) {
          ...
          $(this).attr("data-id", newQuestionNumber);
          $(this).find("label").text(`Q${newQuestionNumber}:`);
          $(this).find("input[id^='question_title_']").attr("id", `question_title_${newQuestionNumber}`);
          $(this).find("input[name^='questions']").attr("name", `questions[${newQuestionNumber}][title]`);
          $(this).find(".delete-question").attr("data-id", newQuestionNumber);
          $(this).find(".options-container").attr("id", `optionsContainer_${newQuestionNumber}`);
          $(this).find(".addOption").attr("data-question-id", newQuestionNumber);
      });
  }

  function updateOptionNumbers(questionId) {
      $(`#optionsContainer_${questionId} .option`).each(function (index) {
          const newOptionNumber = index + 1;
          $(this).attr("data-id", newOptionNumber);
          $(this).find("input[id^='option_text_']").attr("id", `option_text_${questionId}_${newOptionNumber}`);
          $(this).find("input[name^='questions']").attr("name", `questions[${questionId}][options][${newOptionNumber}][text]`);
          $(this).find(".option-checkmark").attr("data-option-id", newOptionNumber);
          $(this).find(".delete-option").attr("data-option-id", newOptionNumber);
          $(this).find("input[id^='is_correct_']").attr("id", `is_correct_${questionId}_${newOptionNumber}`);
      });
  }
